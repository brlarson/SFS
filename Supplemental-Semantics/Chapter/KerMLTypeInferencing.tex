...
multiplicity \texttt{[0]}.

\texttt{~~df\_feature\_inherit\_range\_merge : $\forall$ \{Z W1 W2 f X1 X2 : Element\} \{l1 u1 l2 u2 : $\mathbb{N}$\},}

\texttt{~~~~Specializes Z W1 $\to$ OwnedFeatureTypedByRange W1 f X1 l1 u1 $\to$}

\texttt{~~~~Specializes Z W2 $\to$ OwnedFeatureTypedByRange W2 f X2 l2 u2 $\to$}
...
\texttt{~~~~(df\_feature\_inherit\_range\_merge hspecTU1 hfU1 hspecTV1 hfV1 hoverlap).1,}

\texttt{~~~~(df\_feature\_inherit\_range\_merge hspecTU1 hfU1 hspecTV1 hfV1 hoverlap).2$\rangle$}
\label{MULTM}
\end{restatable}
