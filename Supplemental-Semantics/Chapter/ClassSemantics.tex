...
\ecaption{Occurrence}  
\begin{lstlisting}
abstract class Occurrence specializes Domain::Interval
    disjoint from DataValue {
  feature :>> openLeft default false { }
  feature :>> openRight default false { }
  feature startShot: Instant[1] :>> lowerBound { }
  feature endShot: Instant[0..1] :>> upperBound {}
  feature middleTimeSlice: Interval[0..1] {
    @Assert{f="<< middleTimeSlice = startShot ,, endShot >>";}
...
\pn Use expr \texttt{getLife} instead of feature \texttt{portionOfLife}.

\ecaption{getLife}  
\begin{lstlisting}
		expr getLife {
			doc /* Life is top-level Occurrence, suboccurrence of no other */
			@Assert{f="<< during(this,result) and
			   not exists p~Occurrence that ContainedBy(result,p) >>";}
...
\pn Use bool \texttt{sameLife} instead of feature \texttt{sameLifeOccurrences}.
...
\pn Use bool \texttt{withoutOccurrence} instead of feature \texttt{withoutOccurrences}.

\ecaption{withoutOccurrence}  
\begin{lstlisting}
    bool withoutOccurrence {
...
\pn Use bool \texttt{predecessor} instead of feature \texttt{predecessors}.
...
\pn Use bool \texttt{successor} instead of feature \texttt{successors}.
...
\pn Use bool \texttt{immediatePredecessor} instead of feature \texttt{immediatePredecessors}.

\ecaption{immediatePredecessor}  
\begin{lstlisting}
    bool immediatePredecessor {
...
\pn Use bool \texttt{immediateSuccessor} instead of feature \texttt{immediateSuccessors}.

\ecaption{immediateSuccessor}  
\begin{lstlisting}
    bool immediateSuccessor {
...
\pn Use bool \texttt{timeEnclosedOccurrence} instead of feature \texttt{timeEnclosedOccurrences}.

\ecaption{timeEnclosedOccurrence}  
\begin{lstlisting}
    bool timeEnclosedOccurrence {
      doc /* SFS:  true when o during this */
      in o : Occurrence;
      during(o,this)
    }
\end{lstlisting}

\pn Use bool \texttt{timeCoincidentOccurrence} instead of feature \texttt{timeCoincidentOccurrences}.

\ecaption{timeCoincidentOccurrence}  
\begin{lstlisting}
		bool timeCoincidentOccurrence {
			doc /* SFS:  true when o coincident this */
			in o : Occurrence;
			coincident(o,this)
		}
\end{lstlisting}

\pn Use bool \texttt{spaceEnclosedOccurrence} instead of feature \texttt{spaceEnclosedOccurrences}.

\ecaption{spaceEnclosedOccurrence}  
\begin{lstlisting}
		bool spaceEnclosedOccurrence {
			doc /* SFS:  true when this contains o */
			in o : Occurrence;
			RegionContainment(Location(this),Location(o))
		}
\end{lstlisting}

\pn Use bool \texttt{spaceTimeEnclosedOccurrence} instead of feature \texttt{spaceTimeEnclosedOccurrences}.

\ecaption{spaceTimeEnclosedOccurrence}  
\begin{lstlisting}
		bool spaceTimeEnclosedOccurrence {
...
\pn Use bool \texttt{spaceTimeEnclosedPoint} instead of feature \texttt{spaceTimeEnclosedPoints}.

\ecaption{spaceTimeEnclosedPoint}  
\begin{lstlisting}
		bool spaceTimeEnclosedPoint {
...
\pn Bool \texttt{spaceCoincidentOccurrence} is used by \texttt{spaceTimeCoincidentOccurrence}.

\ecaption{spaceCoincidentOccurrence}  
\begin{lstlisting}
		bool spaceCoincidentOccurrence {
			doc /* SFS:  true when this and o are space coincident */
			in o : Occurrence;
			RegionContainment(Location(o),Location(this)) and
			  RegionContainment(Location(this),Location(o))
		}
\end{lstlisting}

\pn Use bool \texttt{spaceTimeCoincidentOccurrence} instead of feature \texttt{spaceTimeCoincidentOccur\-rences}.

\ecaption{spaceTimeCoincidentOccurrence}  
\begin{lstlisting}
		bool spaceTimeCoincidentOccurrence {
...
\pn Use bool \texttt{outsideOfOccurrence} instead of feature \texttt{outsideOfOccurrences}.

\ecaption{outsideOfOccurrence}  
\begin{lstlisting}
	  bool outsideOfOccurrence {
			doc /* SFS:  true when region of this does not overlap region of o */
			in o : Occurrence;
			not RegionOverlap(Location(this),Location(o))
		}
\end{lstlisting}

\pn Use bool \texttt{justOutsideOfOccurrence} instead of feature \texttt{justOutsideOfOccurrences}.

\ecaption{justOutsideOfOccurrence}  
\begin{lstlisting}
		bool justOutsideOfOccurrence {
			doc /* SFS:  true when this is just outside of o */
			in o : Occurrence;
			ExternallyConnected(Location(this),Location(o))
		}
\end{lstlisting}

\pn Use bool \texttt{matesWithOccurrence} instead of feature \texttt{matesWithOccurrences}.

\ecaption{matesWithOccurrence}  
\begin{lstlisting}
		bool matesWithOccurrence {
			doc /* SFS:  true when this is just outside of o and they touch without overlapping */
			in o : Occurrence;
...
\begin{lstlisting}
      ...
      @Assert{f="<< forall s~Occurrence in suboccurrences are during(s,self)"+
			  " and ContainedBy(s,self) >>";}
...
thing".  In the current library, \texttt{startShot}, \texttt{endShot}, and \texttt{middleTimeSlice} are typed as \texttt{Instant}/\texttt{Interval} rather than \texttt{Occurrence} (see the \texttt{Occurrence} declaration above), so they are no longer declared \texttt{portion}; the argument above still motivates why they would be, were they occurrence-valued.
...
\pn \texttt{PortionOf}, \texttt{TimeSliceOf}, and \texttt{SnapshotOf} are unnecessary (as are \texttt{SpaceSliceOf} and \texttt{SpaceShotOf}, for the same reason). \texttt{Within}, \texttt{WithinBoth}, and \texttt{Without} remain in the library and gain temporal and/or regional semantics along with the associations below.
...
\begin{lstlisting}
	assoc HappensDuring specializes HappensLink {
		...
		* HappensDuring asserts that the shorterOccurrence happens during the longerOccurrence. */
		@Assert{f="<< during(shorterOccurrence, longerOccurrence) >>";}
...
\begin{lstlisting}
	assoc  HappensWhile specializes HappensDuring {
		 ...
		 */
		@Assert{f="<< coincident(thisOccurrence, thatOccurrence) >>";}
		end feature thisOccurrence redefines shorterOccurrence;
		end feature thatOccurrence redefines longerOccurrence;
	}
\end{lstlisting}
...
\begin{lstlisting}
	assoc HappensBefore specializes HappensLink 
	  {
		@Assert{f="<< precedes(earlierOccurrence,laterOccurrence) >>";}
...
\begin{lstlisting}
	assoc HappensJustBefore specializes HappensBefore {
		@Assert{f="<< nearlyMeets(earlierOccurrence,laterOccurrence) >>";}
...
\begin{lstlisting}
	assoc InsideOf specializes SpaceLink {
		@Assert{f="<< RegionContainment(Location(smallerSpace), Location(largerSpace)) >>";}
...
\begin{lstlisting}
	assoc OutsideOf specializes SpaceLink 
	  {
		@Assert{f="<< not RegionOverlap(Location(separateSpaceToo), Location(separateSpace)) >>";}
...
\begin{lstlisting}
	assoc JustOutsideOf specializes OutsideOf {
		 ...
		 */
		@Assert{f="<< FilmConnected(separateSpaceToo, separateSpace) >>";}
...
\begin{lstlisting}
	assoc InnerSpaceOf specializes SpaceLink {
		doc
		/*
		 * InnerSpaceOf is an OutsideOf asserting that the space surrounded by an inner space
		 * boundary of one occurrence (outer space) is completely occupied by another occurrence
		 * (inner space).
		 */
		@Assert{f="<< Location(innerSpace) = RegionInterior(Location(outerSpace)) >>";}
		end feature outerSpace: Occurrence redefines sourceOccurrence;
		end feature innerSpace: Occurrence redefines targetOccurrence 
		   crosses outerSpace.innerSpaceOccurrences;
	}
\end{lstlisting}
