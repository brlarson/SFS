...
\begin{gather}
\langle~\textbf{classifier},\texttt{"OurBikes\textbackslash\#a"}, \{x~\vert ~(~ x \in \texttt{OurBikes}~\wedge~\texttt{OurBikes}\in V_C~ \wedge \notag \\
 ~~~  \texttt{OurBikes}=\texttt{"MyBike\textbackslash\#b"}\cup\texttt{YourBike}~\wedge~\#\texttt{OurBikes}=2~)~\}~\rangle
\end{gather}
...
The ZFC-element for $E$ in class \texttt{"OurBikes"} is \texttt{"OurBikes\textbackslash\#a"}, which by transitivity is also \texttt{"YourBike\textbackslash\#e"}, \texttt{"Bicycle\textbackslash\#f"}, and \texttt{"Vehicle\textbackslash\#g"}.
...
\begin{gather}
\{~\langle \mathbf{classifier},w,C\rangle~\vert~w=W(\texttt{"Bicycle"})~\wedge~y_1,y_2\in\texttt{"Wheel"}~\wedge~y_1\neq y_2~\wedge \notag \\
~~~C~=~\{\langle \texttt{"rollsOn"},R\rangle~\vert~R=\{~\langle w,y_1\rangle,\langle w,y_2\rangle~\}~\}~\}
\label{eq:bicycle-rollson}
\end{gather}

$y_1\neq y_2$ is required so that $R$'s cardinality is exactly $2$, matching \texttt{rollsOn}'s declared
multiplicity \\(\ref{eq:df-fixedfeaturemultiplicity}); \texttt{holdsWheel} contributes nothing to $C$,
having already been instantiated at multiplicity $[0]$ above.
