...
\begin{restatable}{leanaxiom}{birthLean}~\leanTitle{birth}~ Define Birth time in Lean: a primitive (no
construction of ``the first instant a exists'' is given, only the characterizing property
\texttt{birthIff} below). Returns \texttt{Time}, not bare \texttt{Instant}: an occurrence's birth is itself one
of the instants under discussion.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{birth} : Occurrence $\to$ Time}
\label{birth}
\end{restatable}

\begin{restatable}{leanaxiom}{birthIff}~\leanTitle{birthIff}~ Define Birth time's characterizing property in
Lean.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{birthIff} \{A : Occurrence\} \{t0 : Time\} :}
...
\begin{restatable}{leanaxiom}{deathLean}~\leanTitle{death}~ Define Death time in Lean: a primitive, same
reasoning as \texttt{birth} above, but \texttt{Option}-valued (2026-08-25): \texttt{Occurrences.kerml}'s own
\texttt{endShot: Instant[0..1]} means an occurrence may not have ended by \texttt{now}. A total \texttt{death}
forced every occurrence, including one that never exists, to have \emph{some} death instant -- inconsistent,
the same defect \S\ref{sec:bldefinedness} generalizes and fixes.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{death} : Occurrence $\to$ Option Time}
\label{death}
\end{restatable}

\begin{restatable}{leanaxiom}{deathIff}~\leanTitle{deathIff}~ Define Death time's characterizing property in
Lean, restated for \texttt{death}'s \texttt{Option} codomain: only characterizes \texttt{death A = some t0},
saying nothing about \texttt{death A = none} (an ongoing occurrence).

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{deathIff} \{A : Occurrence\} \{t0 : Time\} :}
