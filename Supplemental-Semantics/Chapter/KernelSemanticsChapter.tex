...
\begin{restatable}{leanaxiom}{nowLean}~\leanTitle{now}~ Define the present instant in Lean: a fixed
nonnegative real, the end of recorded time.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{now} : $\mathbb{R}$}

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{now\_nonneg} : 0 $\le$ now}
\label{now}
\end{restatable}
...
\pn \texttt{birth} and \texttt{death} are defined together in Lean, class-based (2026-08-26),
porting a general repair recipe from toy \texttt{$\mathbb{N}$} to the real \texttt{Instant}/\texttt{Time}/
\texttt{TIME}: guard each characterizing law with its presupposition, keep the function total (or here
\texttt{Option}-valued) with an unconstrained/undefined value when the presupposition fails, then \emph{prove}
a model exists rather than assume one. The port is not mechanical -- \texttt{$\mathbb{N}$} is well-ordered for
free, so any nonempty subset has a least element with no extra assumption; an arbitrary subset of the dense
continuum \texttt{Time} need not (e.g.\ $\{t \mid t.\mathrm{val} < 0.5\}$ has an infimum but no attained
minimum) -- so \texttt{birth}'s guard needs one further, explicitly-named structural axiom
(\ref{occurrenceHasFirstInstant} below) that the $\mathbb{N}$ case gets for free from well-ordering.
\texttt{death} needs no such extra axiom: its guard is self-certifying, the same reasoning that already
justified \texttt{endShot}'s \texttt{Option} redesign.

\begin{restatable}{leandefinition}{LifetimesLean}~\leanTitle{Lifetimes}~ Define the \texttt{birth}/\texttt{death}
class in Lean: \texttt{birth} stays total (\texttt{startShot} stays mandatory), \texttt{death} stays
\texttt{Option}-valued (\texttt{endShot} does not); \texttt{birthIff} is guarded on \texttt{A} existing
somewhere (unconditional would force even a never-existing \texttt{A} to have \emph{some} birth instant);
\texttt{deathIff} needs no such guard -- the \texttt{Option} codomain already \emph{is} the guard.
...
\texttt{\textcolor{leancolor}{export} Lifetimes (birth death birthIff deathIff)}
\label{Lifetimes}
\end{restatable}

\begin{restatable}{leanaxiom}{occurrenceHasFirstInstantLean}~\leanTitle{occurrenceHasFirstInstant}~ The one new
trusted ingredient this reformation needs: every \texttt{Occurrence} that exists anywhere has a \emph{first}
instant of existence, not merely an infimum of existence times -- formalizes \texttt{Occurrences.kerml}'s
mandatory \texttt{startShot: Instant[1]} (unlike \texttt{endShot}, never weakened to \texttt{[0..1]}).

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{occurrenceHasFirstInstant} (A : Occurrence)}

\texttt{~~(\_h : $\exists$ s, existsAt A s) : $\exists$ $\tau$, IsLeast \{t : Time $\mid$ existsAt A t\} $\tau$}
\label{occurrenceHasFirstInstant}
\end{restatable}

\begin{restatable}{leantheorem}{birthExistsLean}~\leanTitle{birth\_exists}~ A total selector satisfying
\texttt{birthIff}'s guarded spec exists for every occurrence: \ref{occurrenceHasFirstInstant}'s witness when
\texttt{A} exists somewhere, an unconstrained junk value otherwise. Proof omitted here (case split on
existence, direct from \ref{occurrenceHasFirstInstant} in the positive case).
...
built via \texttt{Classical.choose} over \ref{birth_exists}/\ref{death_exists} -- exactly the construction
that would be impossible without \texttt{birthIff}/\texttt{deathIff}'s guards. A plain (not locally-scoped)
\texttt{instance}, so every downstream use of \texttt{birth}/\texttt{death}/\texttt{birthIff}/\texttt{deathIff}
elsewhere resolves to it automatically. \texttt{birthIff}/\texttt{deathIff}'s own field proofs (via
...
\emph{one} new ingredient, \ref{occurrenceHasFirstInstant} -- nothing else.
...
\begin{restatable}{leanaxiom}{openLeftRight}~\leanTitle{openLeft / openRight}~ \texttt{Interval}'s own
...
\texttt{df-nearlymeets} was added to \texttt{SFS.mm}.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{openLeft} : Occurrence $\to$ Prop}

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{openRight} : Occurrence $\to$ Prop}
\label{openLeft}
\end{restatable}
...
\begin{restatable}{leandefinition}{nearlyMeetsLean}~\leanTitle{nearlyMeets}~ Define nearlymeets in Lean. The
\texttt{next (death A) (birth B)} disjunct is, per \texttt{next\_dense} (\S3.1.1), provably always
\texttt{False} here (\texttt{next} is vacuous on dense $\mathbb{R}$), so this reduces semantically to just the
open-boundary disjunct --- see \texttt{nearlyMeets\_iff} below. Included literally anyway, matching
\texttt{SFS.mm}'s own formula rather than silently dropping the now-vacuous part.

\texttt{\textcolor{leancolor}{def} \textcolor{leanyellow}{nearlyMeets} (A B : Occurrence) : Prop :=}

\texttt{~~(birth B = death A $\wedge$ (openRight A $\vee$ openLeft B)) $\vee$ next (death A) (birth B)}
\label{nearlyMeets}
\end{restatable}

\begin{restatable}{leantheorem}{nearlyMeetsIff}~\leanTitle{nearlyMeets\_iff}~ Free consequence of
\texttt{next\_dense}, not a fact \texttt{SFS.mm}/\texttt{Allen.kerml} state themselves: \texttt{nearlyMeets}
collapses to its open-boundary disjunct alone.

\texttt{\textcolor{leancolor}{theorem} \textcolor{leanyellow}{nearlyMeets\_iff} (A B : Occurrence) :}

\texttt{~~nearlyMeets A B $\leftrightarrow$ (birth B = death A $\wedge$ (openRight A $\vee$ openLeft B)) :=}

\texttt{~~\textcolor{leancolor}{by} unfold nearlyMeets; simp [next\_dense]}
\label{nearlyMeets_iff}
\end{restatable}
...
\texttt{featureAccess} standing for KerML's \texttt{d::f} feature navigation.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{featureAccess} : Occurrence $\to$ Element $\to$ Occurrence}
...
\begin{restatable}{leanaxiom}{GetChangeToTrueLean}~\leanTitle{GetChangeToTrue}~ Define \texttt{GetChangeToTrue}
...
standing for the Boolean literal \texttt{true}.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{featureAccessProp} : Occurrence $\to$ Element $\to$ TProp}
...
\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{trueItem} : Set Item}
...
\begin{restatable}{leanaxiom}{GetChangeToFalseLean}~\leanTitle{GetChangeToFalse}~ Define
\texttt{GetChangeToFalse} in Lean, mirror of \texttt{GetChangeToTrue} above. \texttt{falseItem} is a fresh
opaque canonical value standing for the Boolean literal \texttt{false}.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{falseItem} : Set Item}
...
\begin{restatable}{leanaxiom}{changedLean}~\leanTitle{changed}~ Define \texttt{changed} in Lean: a primitive
(no construction of ``the most recent instant before $\tau_1$ that $d$'s value differed'' is given, only the
characterizing property \texttt{changedIff} below), same treatment as \texttt{birth}/\texttt{death}.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{changed} : Occurrence $\to$ Time $\to$ Time}
\label{changed}
\end{restatable}

\begin{restatable}{leanaxiom}{changedIff}~\leanTitle{changedIff}~ Define \texttt{changed}'s characterizing
property in Lean. Same caveat as \texttt{birth}/\texttt{death}: this is a total function even though the
book's own \texttt{df-bl.changed} only partially characterizes it.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{changedIff} \{d : Occurrence\} \{t1 t2 : Time\} :}

\texttt{~~changed d t1 = t2 $\leftrightarrow$}

\texttt{~~~~(t2.val $\prec$ t1.val $\wedge$ (interpValAt d t1 $\ne$ interpValAt d t2 $\vee$ t2.val = 0) $\wedge$}

\texttt{~~~~~$\forall$ t : Time, t.val $\in$ Set.Ioo t2.val t1.val $\to$ interpValAt d t = interpValAt d t2)}
\label{changedIff}
\end{restatable}
...
\begin{restatable}{leanaxiom}{timeofLean}~\leanTitle{timeof}~ Define \texttt{timeof} in Lean: a primitive, same
treatment as \texttt{changed}/\texttt{birth}/\texttt{death} above. The book's own prose notes
\texttt{timeof}$(\varphi)$ should read as the empty set when $\varphi$ never holds for any $\tau\in\mathbf{T}$
--- a genuine ZFC partial-function reading this file's total \texttt{Time}-valued \texttt{timeof} doesn't
literally capture (same simplification already accepted for \texttt{birth}/\texttt{death}), so
\texttt{timeofIff} below is a characterization, not a full definition.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{timeof} : TProp $\to$ Time}
\label{timeof}
\end{restatable}

\begin{restatable}{leanaxiom}{timeofIff}~\leanTitle{timeofIff}~ Define \texttt{timeof}'s characterizing
property in Lean.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{timeofIff} \{$\varphi$ : TProp\} \{t1 : Time\} :}

\texttt{~~timeof $\varphi$ = t1 $\leftrightarrow$ (interpAt $\varphi$ t1 $\wedge$ $\forall$ t : Time, t.val
$\in$ Set.Ico (0 : Instant) t1.val $\to$ $\lnot$ interpAt $\varphi$ t)}
\label{timeofIff}
\end{restatable}
