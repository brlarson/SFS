...
no attained minimum) -- so \texttt{birth}'s (and \texttt{timeof}'s) guard needs one further, explicitly-named
structural axiom (\ref{hasFirstInstant} below) that the $\mathbb{N}$ case gets for free from well-ordering.
...
\begin{restatable}{leanaxiom}{hasFirstInstantLean}~\leanTitle{hasFirstInstant}~ The one new trusted ingredient
this reformation needs, stated generically (not fixed to \texttt{existsAt A}) so it serves both \texttt{birth}
(\texttt{P := existsAt A}, formalizing \texttt{Occurrences.kerml}'s mandatory \texttt{startShot: Instant[1]})
and \texttt{timeof} below (\texttt{P := interpAt $\varphi$}) without a near-duplicate axiom for each: any
\texttt{Time $\to$ Prop} predicate that holds anywhere has a \emph{first} instant where it holds, not merely
an infimum of the instants where it holds.

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{hasFirstInstant} (P : Time $\to$ Prop)}

\texttt{~~(\_h : $\exists$ s, P s) : $\exists$ $\tau$, IsLeast \{t : Time $\mid$ P t\} $\tau$}
\label{hasFirstInstant}
\end{restatable}

\begin{restatable}{leantheorem}{birthExistsLean}~\leanTitle{birth\_exists}~ A total selector satisfying
\texttt{birthIff}'s guarded spec exists for every occurrence: \ref{hasFirstInstant}'s witness (at
\texttt{P := existsAt A}) when \texttt{A} exists somewhere, an unconstrained junk value otherwise. Proof
omitted here (case split on existence, direct from \ref{hasFirstInstant} in the positive case).
...
\ref{hasFirstInstant}, carries over unchanged.
...
\emph{one} new ingredient, \ref{hasFirstInstant} -- reused for both \texttt{birth} and \texttt{timeof} --
nothing else.
...
characterizing property \texttt{changedIff} below). \textbf{Not} given the same guard-and-prove treatment as
\texttt{birth}/\texttt{death}/\texttt{timeof} (2026-08-26 investigation): unlike those, \texttt{changedIff}'s
presupposition isn't merely ``sometimes fails at existence edge cases.'' For an ordinary step-function
\texttt{Occurrence} (e.g.\ $d$ constant $A$ before $0.5$, constant $B$ from $0.5$ on) and any $\tau_1$ that
isn't itself a fresh transition instant for $d$ (e.g.\ $\tau_1=0.8$), \emph{no} $\tau_2$ satisfies
\eqref{eq:df-bl.changed}'s three conditions jointly: the exact transition point fails the ``differs'' disjunct
(its value already matches $d(\tau_1)$), and every earlier $\tau_2$ fails the open-interval constancy clause
(the interval $(\tau_2,\tau_1)$ straddles the transition). This looks like a real gap in
\texttt{df-bl.changed}'s own formalization (likely inherited from \texttt{SFS.mm}), not the
\texttt{birth}/\texttt{death}-style ``no least element in a dense order'' presupposition gap. A promising fix
(finite variability -- only finitely many instants in $[0,\mathsf{now}]$ are actual value-changes, sidestepping
the dense-order completeness problem entirely since finite sets always have an attained maximum) has been
proposed but not yet implemented; left here as a known, unresolved issue rather than guessed at.
...
property in Lean. See \texttt{changed}'s own doc note above: this file's \texttt{changed} stays a total,
unguarded function -- not merely because ``the book's own \texttt{df-bl.changed} only partially characterizes
it'' (the original framing), but because the guard-and-prove treatment doesn't obviously apply as-is.
...
for the field declarations, \ref{timeof_exists} for the guarded existence proof (reusing \ref{hasFirstInstant},
the same new axiom \texttt{birth} needs), and \ref{model} for the real, audited construction.
