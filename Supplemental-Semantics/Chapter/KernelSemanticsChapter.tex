...
\texttt{birth B} are consecutive shared ticks, so there is no longer an unconditional collapse to just the
open-boundary disjunct to state (the old \texttt{nearlyMeets\_iff} is retired along with the vacuity it
relied on).

\texttt{\textcolor{leancolor}{noncomputable def} \textcolor{leanyellow}{nearlyMeets} (A B : Occurrence) : Option Prop :=}

\texttt{~~(death A).map fun tA $\Rightarrow$ (birth B = tA $\wedge$ (openRight A $\vee$ openLeft B)) $\vee$ next tA (birth B)}
\label{nearlyMeets}
\end{restatable}
