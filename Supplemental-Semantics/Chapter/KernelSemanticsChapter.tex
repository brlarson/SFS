...
$X ~s~ Y$ & X  \bemph{starts} Y & $\texttt{birth}(X) = \texttt{birth}(Y)~\wedge~\texttt{effectiveEnd}(Y) \prec \texttt{effectiveEnd}(X)$ & \textidx{starts}(X,Y) \\ \hline
$X ~d~ Y$ & X  \bemph{during} Y & $\texttt{birth}(Y) \preccurlyeq \texttt{birth}(X)~\wedge~\texttt{effectiveEnd}(X) \preccurlyeq \texttt{effectiveEnd}(Y)$ & \textidx{during}(X,Y)\\ \hline
$X ~f~ Y$ & X  \bemph{finishes} Y & $\texttt{birth}(Y) \prec \texttt{birth}(X)~\wedge~\texttt{effectiveEnd}(X) = \texttt{effectiveEnd}(Y)$ & \textidx{finishes}(X,Y)\\ \hline
$X ~=~ Y$ & X  \bemph{equals} Y & $\texttt{birth}(Y) = \texttt{birth}(X)~\wedge~\texttt{effectiveEnd}(X) = \texttt{effectiveEnd}(Y)$ & \textidx{coincident}(X,Y) \\ \hline
\end{tabular}
\end{center}
