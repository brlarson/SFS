...
\pn \texttt{df-model} is book-original, with no \texttt{SFS.mm} formalization of its own (confirmed absent from
\texttt{SFS.mm} by grep, and by \texttt{SFS.lean}'s own doc comment at the \texttt{Model} structure below) --
$\mathfrak{M}$'s tuple packages the surrounding informal exposition, not a restated Metamath axiom or theorem.
...
\pn  $\mathbf{I}\lsem d \rsem$ is the same at every time $\tau\in\mathbf{T}$:
 ...
 \begin{restatable}{definition}{dfblmodels}~\blTitle{ax-bl.models}~ Define interpretation of predicate for
any $\tau \in \mathbf{T}$
\begin{align}
\vdash \tau \in \mathbf{T}\quad\Rightarrow\quad \vdash ( \mathbf{I} \lsem
    \mw{\varphi} \rsem \rightarrow \mathbf{I} \lsem \mw{\varphi} , \tau \rsem
    )\label{eq:df-bl.models}\tag{ax-bl.models}
\end{align}
\end{restatable}
...
\begin{restatable}{definition}{dfblmodelsc}~\blTitle{ax-bl.modelsc}~ Define interpretation of values over
any $\tau \in \mathbf{T}$
\begin{align}
\vdash \tau \in \mathbf{T}\quad\Rightarrow\quad \vdash ( \mathbf{I} \lsem
    \mc{A} \rsem = \mc{B} \rightarrow \mathbf{I} \lsem \mc{A} , \tau \rsem =
    \mc{B} )\label{eq:df-bl.modelsc}\tag{ax-bl.modelsc}
\end{align}
\end{restatable}

\pn \texttt{ax-bl.modelsc} is only meaningful when $\mc{A}$ is actually constant, and is not needed to make
...
class-based (2026-08-26), the simplest instance of the general repair recipe explained in full below at
\texttt{Lifetimes} (\ref{Lifetimes}) -- \texttt{now} needs no guard the way \texttt{birth}/\texttt{death}/\texttt{timeof}
...
\pn Under the necessity ($\Box$) reading, \texttt{interp} distributes over $\wedge_t$/$\forall$ both ways, but --- as with any normal modal box operator --- only \emph{one} direction is valid for $\vee_t$/$\neg_t$/$\to_t$/$\leftrightarrow_t$/$\exists$ in general (the classic K-axiom asymmetry). \texttt{SFS.mm} itself now states \texttt{df-bl.or}/\texttt{df-bl.not}/\texttt{df-bl.im}/\texttt{df-bl.bi}/\texttt{bl.ex} as one-directional (sound-direction-only) implications, not biconditionals --- each carries its own comment citing the matching \texttt{SFS.lean} soundness theorem and noting the converse is false in general; \texttt{SFS.mm} was updated to this stance after \texttt{SFS.lean}'s own two-instant counterexamples established it. \texttt{SFS.lean} proves only the sound direction of each, witnessing the unsound direction false in general via two-instant counterexamples (not restated as axioms) --- the two sources now agree, not diverge.
...
\pn Temporal values also support pointwise-lifted relations, matching \texttt{SFS.mm}'s \texttt{df-bl.eq}, \texttt{df-bl.eqt}, \texttt{df-bl.lt}, 
\texttt{df-bl.ltt}, \texttt{df-bl.am}, and \texttt{df-bl.amt}:
...
\begin{restatable}{definition}{dfexists}~\blTitle{df-exists}~ Define element existence predicate
\begin{align}
\vdash (~ \texttt{ex}(c,\tau) \leftrightarrow \mathbf{I}\lsem c,\tau\rsem\neq\emptyclass~)
\label{eq:df-exists}\tag{df-exists}
\end{align}
\end{restatable}
...
\begin{restatable}{definition}{dflifetime}~\blTitle{df-lifetime}~Define Lifetime.
\begin{align}
\vdash \texttt{life}(c)~=~ (\texttt{birth}(c)~[,]~\texttt{death}(c))
\label{eq:df-lifetime}\tag{df-lifetime}
\end{align}
\end{restatable}
...
\texttt{SFS.mm}'s own \texttt{df-lifetime} (\eqref{eq:df-lifetime} above), which is left unchanged, still
...
~\wedge~ \texttt{openLeft}(X) = \texttt{openLeft}(Y)
...
~\wedge~ \texttt{openRight}(X) = \texttt{openRight}(Y)
...
~\wedge~ \texttt{openLeft}(X) = \texttt{openLeft}(Y) \wedge \texttt{openRight}(X) = \texttt{openRight}(Y)
...
\begin{restatable}{definition}{blchanged}~\blTitle{df-bl.changed}~ Time when a variable value most recently
changed. Redefined 2026-08-26 in \texttt{SFS.mm} itself (its own comment there calls the previous version
``close to vacuous for ordinary step-function occ''): grounded in \texttt{isTick} rather than an open real
interval, matching \texttt{finitePartition}'s finite shared ticks (\ref{finitePartition}).
\begin{align}
\vdash (~ \texttt{changed}(d,\tau_1)~=~\tau_2 \leftrightarrow \notag \\
 (~( \texttt{isTick}(\tau_2) \wedge \tau_2 \preccurlyeq \tau_1 ) \wedge \notag \\
 ~\forall \tau~(~( \texttt{isTick}(\tau) \wedge \tau_2 \preccurlyeq \tau \wedge \tau \preccurlyeq \tau_1 ) \rightarrow \mathbf{I}\lsem d,\tau\rsem = \mathbf{I}\lsem d,\tau_1\rsem~) \wedge \notag \\
 ~( \tau_2 = 0 \vee \exists \tau_3~(~\texttt{next}(\tau_3,\tau_2) \wedge \mathbf{I}\lsem d,\tau_3\rsem \neq \mathbf{I}\lsem d,\tau_1\rsem~)~)~)~)
...
satisfied the \emph{pre}-2026-08-26 \texttt{df-bl.changed}'s three conditions jointly ($\tau_2\prec\tau_1$;
$\mathbf{I}\lsem d,\tau_1\rsem \neq \mathbf{I}\lsem d,\tau_2\rsem \vee \tau_2=0$; and open-interval constancy
on $(\tau_2,\tau_1)$, the shape \eqref{eq:df-bl.changed} had before this same-day redesign) -- the exact
transition point failed the ``differs'' disjunct (its value already matched $d(\tau_1)$), and every earlier
$\tau_2$ failed the open-interval constancy clause. Not the \texttt{birth}/\texttt{death}-style ``no least
element in a dense order'' gap; a real gap in that older formalization (likely inherited from
\texttt{SFS.mm}, which has since redefined \texttt{df-bl.changed} to the \texttt{isTick}-grounded form
shown above -- see that definition's own note). \texttt{changed d t1} is now: among the ticks strictly before \texttt{t1} where
...
\texttt{Time} is deliberately \emph{not} added to $SV$ as a bare \texttt{Real}-specializing \texttt{datatype}
--- \texttt{Domain::Instant} instead specializes \texttt{ISQSpaceTime::TimeValue} (an ISQ physical quantity,
not a \texttt{ScalarValues} scalar), so that a time instant carries a unit and dimension rather than being an
unadorned real number. See \S\ref{sec:fortime}.
...
\begin{align}
\vdash SV = \mathbb{N} \cup \mathbb{N}_0 \cup \mathbb{Z} \cup \mathbb{Q} \cup \mathbb{R} \cup \mathbb{C} \cup \mathbb{B} \cup \mathbb{S} \label{eq:df-scalarvalues}\tag{df-scalarvalues}
\end{align}
\end{restatable}
...
The \texttt{VectorValues} library provides \texttt{VectorValue} (abstract), \texttt{NumericalVectorValue}
(\texttt{:>} \texttt{VectorValue}, \texttt{Collections::Array}, redefining features \texttt{elements} and
\texttt{dimension}, dimension left open --- \texttt{[0..1]}, empty by default, collapsing to a rank-0 scalar),
\texttt{CartesianVectorValue} (\texttt{:>} \texttt{NumericalVectorValue}, elements restricted to \texttt{Real}), and
\texttt{ThreeVectorValue}/\texttt{CartesianThreeVectorValue}, which fix \texttt{dimension} at exactly 3. Only
dimension 3 is named concretely this way; 1 and 2 are not yet given their own specializations.
...
\pn \textbf{checkDataTypeElement:}~~ The \texttt{elements} of a \texttt{Collection} must specialize \texttt{DataValue}.\\
\texttt{elements->specializesFromLibrary('Base::DataValue')}
