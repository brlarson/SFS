...
\begin{definition}\blTitle{df-next} {Define the next instant (successor, no delta)} Redefined 2026-08-26 in
\texttt{SFS.mm} itself (ported from \texttt{Lean4SFS/SFS.lean}'s own redesign): the original order-theoretic
form below is provably vacuous under unrestricted density (\ref{eq:bl.nextdense}); grounding it in
\texttt{isTick} instead (matching \ref{finitePartition}'s finite shared ticks) is what makes it satisfiable.
\begin{align}
\vdash (~ \texttt{next}( \tau_1, \tau_2 ) \leftrightarrow ( ( \texttt{isTick}(\tau_1) \wedge \texttt{isTick}(\tau_2) ) \wedge \tau_1 \prec \tau_2 ) \wedge \notag \\
~\lnot \exists \tau~( ( \texttt{isTick}(\tau) \wedge \tau_1 \prec \tau ) \wedge \tau \prec \tau_2 )~)
    \label{eq:df-next}\tag{df-next}
\end{align}
\end{definition}
...
\pn \bemph{Retired in \texttt{SFS.mm}}: \texttt{bl.nextdense} below was a real, formalized \texttt{SFS.mm}
theorem about the \emph{pre}-2026-08-26 \texttt{next} (the pure order-theoretic form, no \texttt{isTick}) --
it no longer applies to \texttt{next}'s current, \texttt{isTick}-grounded meaning (\eqref{eq:df-next} above),
which is genuinely satisfiable under density, and \texttt{SFS.mm} removed the statement entirely rather than
leave it stated about a form \texttt{next} no longer has. Restated here, unlabeled by \texttt{SFS.mm}, purely
for the historical argument it makes: this is exactly \emph{why} the redesign was needed.

\begin{lemma}\blTitle{bl.nextdense} {next is empty under unrestricted density (retired in SFS.mm)}
\begin{align}
...
\noindent\emph{Proof sketch (informal; SFS.mm's own now-removed proof followed the same argument):}
Instantiate the density hypothesis at any $\tau_1,\tau_2$ with $\texttt{next}(\tau_1,\tau_2)$ (which gives
$\tau_1 \prec \tau_2$): density yields an instant strictly between them, directly contradicting the
non-existence conjunct of \eqref{eq:df-next} (in its pre-\texttt{isTick} form, discussed there).
