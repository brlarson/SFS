...
\begin{restatable}{leandefinition}{nextOLean}~\leanTitle{nextO}~ \textit{next} lifted to \texttt{Option Time}
arguments, needed for \textit{nearlymeets}'s own \eqref{eq:df-nearlymeets} (\S\ref{sec:allen},
\ref{nearlyMeets}). \emph{False, not undefined}, when either argument is absent -- a deliberate departure
from the Strong Kleene reading \S\ref{sec:bldefinedness} gives every other partial predicate here:
\textit{next} asks whether a specific consecutive-tick relationship holds between two instants, and ``one of
the instants doesn't exist yet'' makes that relationship false, not merely unknown (contrast a bare
comparison like $\texttt{death}(X)\prec\texttt{birth}(Y)$, where an unfinished $X$ genuinely leaves the
comparison undecided rather than false).

\texttt{\textcolor{leancolor}{noncomputable def} \textcolor{leanyellow}{nextO} (t1 t2 : Option Time) : Prop :=}

\texttt{~~\textcolor{leancolor}{match} t1, t2 \textcolor{leancolor}{with}}

\texttt{~~| some t1', some t2' $\Rightarrow$ next t1' t2'}

\texttt{~~| \_, \_ $\Rightarrow$ False}
\label{nextO}
\end{restatable}
...
\pn No new surface syntax is needed to make a formula partiality-aware once one of its function symbols is known to be partial -- \eqref{eq:df-precedes} (\S\ref{sec:allen}) is written exactly the same whether \texttt{death} is total or partial; only the truth value it can now take, and the proof obligations that follow from that, change. $r\!\downarrow$ is available where a formula needs to test definedness explicitly, but none of \S\ref{sec:allen}'s own formulas need it -- they rely on the comparison-atom and Strong Kleene $\wedge$/$\vee$ rules above alone, with one deliberate exception: \eqref{eq:df-nearlymeets}'s own $\textit{next}(\texttt{death}(X),\texttt{birth}(Y))$ disjunct is a predicate symbol, not a comparison atom, and this $\mathsf{PRED}$ apparatus is not itself given a general partiality rule here -- \textit{next} instead supplies its own convention (\ref{nextO}), false rather than $\bot_u$ when an argument is undefined, precisely because it is not a comparison.
