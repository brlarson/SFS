...
\texttt{Behaviors}\index{behavior} are \texttt{Classes} which implicitly specialize \texttt{Performances::Performance} which is an \texttt{Occur\-rence} and \texttt{Virtual} (\S\ref{subsec:virtual}), disjoint from \texttt{Object}.  Because structures specialize \texttt{Object}, behaviors and structures must be disjoint.  A behavior exists in time because it is an \texttt{Occur\-rence}; it has no physical existence of its own because it is \texttt{Virtual}.
...
\texttt{during(focus,TakePicture)}~~\texttt{during(shoot,TakePicture)}\\
...
\pn \texttt{Performance} has both \texttt{enclosedPerformances} and \texttt{subperformances}.  They no
longer seem redundant: \texttt{enclosedPerformances} is the general ``during'' relation, and
\texttt{subperformances} is its \texttt{composite}, \texttt{PartOf}-adding subset --
\texttt{enclosedPerformances} \texttt{subsets} \texttt{subperformances}, not the other way around.

\ecaption{enclosedPerformances}
\begin{lstlisting}
		step enclosedPerformances: Performance[0..*] subsets subperformances {
			doc
			/*
			 * timeEnclosedOccurrences of this Performance that are also Performances.
			 */
			@Assert{f="<< forall ep~Performance in enclosedPerformances are during(ep,thisPerformance) >>";}
		}
\end{lstlisting}

\ecaption{subperformances}
\begin{lstlisting}
composite step subperformances: Performance[0..*] subsets suboccurrences {
	doc
	/*
	 * enclosedPerformances that are composite.
	 */
	@Assert{f="<< forall sp~Occurrence in subperformances are during(sp,self) and PartOf(sp,self) >>";}
...
\begin{lstlisting}
  metaclass Function specializes Classifier {
  	derived var feature expression : Expression[0..*] subsets 'member';
  	derived var feature parameter : Feature[0..*];
  	derived var feature result : Feature[1..1] redefines output;
  }
\end{lstlisting}
