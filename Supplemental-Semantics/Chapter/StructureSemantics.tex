...
struct S specializes Objects::Object {
	feature a : S subsets Objects::objects;
	composite feature b : S subsets Objects::Object::subobjects;
}
\end{lstlisting}

\pn An \texttt{Objects::Object} may have \texttt{subobjects} which are defined using metaphysical mereology (\ref{eq:df-par}).
\ecaption{subobjects}
\begin{lstlisting}
	abstract struct Object specializes Occurrence {
		...
		composite feature subobjects: Object[0..*] subsets objects, suboccurrences
			...
			@Assert{f="<< forall so~Occurrence in subobjects are during(so,self) and PartOf(so,self) >>";}
		}
	}
\end{lstlisting}

\pn All subobjects of an objects are parts of that object (\ref{eq:df-par}).
...
\begin{lstlisting}
		portion structuredSpaceBoundary : StructuredSpaceObject[0..1] subsets spaceBoundary {
			 ...
			 */
			@Assert{f="<< not structuredSpaceBoundary->empty implies"+
...
\pn Physical objects may only contain other physical objects.  Virtual objects are allocated to physical objects.  By expansivity (\ref{eq:df-pec}), parthood implies regional containment.
...
\pn \texttt{bl.containment} is book-original, with no \texttt{SFS.mm} formalization of its own (confirmed absent
from \texttt{SFS.mm} by grep) -- it follows from \texttt{structuredSpaceBoundary}'s own \texttt{@Assert} together
with expansivity (\ref{eq:df-pec}), but is not restated as a Metamath theorem.
...
\begin{lstlisting}
		abstract step enactedPerformances: Performance[0..*] subsets involvingPerformances {
			 ...
			 */
			@Assert{f="<< forall ep~Performance in enactedPerformances are during(ep,self) >>";}
		}
\end{lstlisting}
...
\begin{lstlisting}
		composite step ownedPerformances: Performance[0..*] subsets involvingPerformances {
			@Assert{f="<< forall op~Performance in ownedPerformances are during(op,self)"+
			  " and PartOf(op,self) >>";}
			doc
			/*
