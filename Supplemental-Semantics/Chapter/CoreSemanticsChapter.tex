...
no separate ``is a genuine set" assertion is needed here: \texttt{Set KElement} already
...
\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{Design} : Set (DesignKind $\times$ ID $\times$ Set KElement)}
\label{Design}
\end{restatable}
...
\begin{restatable}{leandefinition}{VT}~\leanTitle{VT}~ Define \texttt{Types} in Lean. Unlike
...
\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{VT} : Set ID}

\texttt{\textcolor{leancolor}{axiom} \textcolor{leanyellow}{df\_types} : $\forall$ x : ID, ($\exists$ y, (DesignKind.type, x, y) $\in$ Design) $\to$ x $\in$ VT}
\label{VT}
\end{restatable}
