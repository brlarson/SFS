...
\begin{restatable}{leandefinition}{mkUid}~\leanTitle{mkUid}~ Define creation of a Unique Identifier from a
Class Identifier by appending a wonce, in Lean. \texttt{wonce} is class-based (2026-08-26), same
...
\texttt{\textcolor{leancolor}{class} \textcolor{leanyellow}{Wonce} \textcolor{leancolor}{where}}

\texttt{~~wonce : String}

\texttt{\textcolor{leancolor}{export} Wonce (wonce)}
...
\pn $\mathrm{Design}$ itself is left fully opaque above (\S2.1.2's own ``all elements in the
database or workspace'' is informal prose, not a formal constraint) --- nothing anywhere in this
formalization actually constrains it beyond \ref{eq:df-type}/\ref{eq:df-types} below. Defining it
directly as the empty set (2026-08-26) makes both of those provable instead of independently
axiomatized, with no loss of content: nothing else ever relies on $\mathrm{Design}$ being
non-empty. This is a formalization convenience, not a claim that real designs have no
elements --- the same status \texttt{Design} already had as an unconstrained opaque primitive,
just with one further (harmless) commitment now attached to it.

\begin{restatable}{definition}{dfdesign}~\blTitle{df-design}~ Define $\mathrm{Design}$.
\begin{align}
\vdash \mathrm{Design} = \emptyset \label{eq:df-design}\tag{df-design}
\end{align}
\end{restatable}

\begin{restatable}{definition}{dftype}~\blTitle{df-type}~ Define \texttt{type}. Now a real theorem,
not an axiom (2026-08-26): with \ref{eq:df-design}, the hypothesis
$\langle \mathsf{TYPE},\mc{A},\mc{C}\rangle \in \mathrm{Design}$ is never satisfiable, so the
conclusion follows regardless of $C$.
\begin{align}
...
\begin{restatable}{leandefinition}{Design}~\leanTitle{Design}~ Define \texttt{type} in Lean.
...
below are the complete translation. Now a field of \texttt{class CoreDesignModel} (\ref{CoreDesignModel}
below) rather than an independent axiom.

\texttt{\textcolor{leancolor}{inductive} \textcolor{leanyellow}{DesignKind}}

\texttt{~~| ofElementKind (k : ElementKind) | type}

\texttt{~~Design : Set (DesignKind $\times$ Element $\times$ Set Element)}
\label{Design}
\end{restatable}
...
\begin{restatable}{definition}{dftypes}~\blTitle{df-types}~ Define \texttt{Types}. Now a real
theorem, not an axiom (2026-08-26), same reasoning as \ref{eq:df-type} above: with
\ref{eq:df-design}, no $y$ can ever satisfy the antecedent, so the whole implication holds
vacuously.
\begin{align}
...
\begin{restatable}{leandefinition}{VT}~\leanTitle{VT}~ Define \texttt{Types} in Lean. \texttt{VT} is
independently primitive from \texttt{Design}, so \texttt{df\_types} needed real proof, not an automatic
Lean-typing shortcut -- both are now fields of \texttt{class CoreDesignModel} below.
\texttt{df\_types} itself moved here from \texttt{SFS.lean} (2026-08-26, folded into this class since
\texttt{Design}/\texttt{VT} are shared, load-bearing primitives for the whole cluster below, not private
to \texttt{df\_types}).

\texttt{~~VT : Set Element}

\texttt{~~df\_types : $\forall$ e : Element, ($\exists$ C : Set Element, (DesignKind.type, e, C) $\in$ Design) $\to$ e $\in$ VT}
\label{VT}
\end{restatable}

\begin{restatable}{leandefinition}{CoreDesignModelLean}~\leanTitle{CoreDesignModel}~ Class-based
(2026-08-26, at direct request, following a full audit of this whole $\sim$45-axiom cluster):
\texttt{Design}/\texttt{VT}/\texttt{Specializes}/\texttt{Unions}/\texttt{Intersects}/\texttt{Differences}/
\texttt{Disjoint}/\texttt{TypeMultiplicityExact}/\texttt{TypeMultiplicityRange}/\texttt{VF}/
\texttt{FeatureContent}/\texttt{FeaturedByDecl}/\texttt{DomainlessDecl}/\texttt{FeatureMemberOf}/
\texttt{OwnedFeatureTypedBy} (4 multiplicity-suffixed forms)/\texttt{MemberOf}/\texttt{OwnedFeatures}/
\texttt{OrderedOn}/\texttt{OrderedDecl}/\texttt{ChainsDecl}/
\texttt{VC} bundled together with every \texttt{df\_*} constraint that ties them to \texttt{Design}
(\texttt{df\_specializes} through \texttt{df\_typebody}, \texttt{df\_classifier},
\texttt{df\_classifier2}) --- one class, not one axiom per primitive/constraint, because almost every
constraint here shares the same hypothesis shape: \texttt{(DesignKind.\_, \_, \_) $\in$ Design} (or
\texttt{FeatureContent \_ \_}, for \texttt{df\_featurechaining}). A single global witness discharges
essentially the whole cluster at once: \texttt{Design := $\emptyset$} and \texttt{FeatureContent := fun
\_ \_ => False} make every one of those hypotheses unconstructible, so every constraint holds
\emph{vacuously} --- checked individually for each constraint, not assumed from the shape alone.
Built via \texttt{Nonempty}/\texttt{Classical.choice} (one concrete named-field term) rather than
\texttt{Classical.choose} over an $\exists$-tuple: at $\sim$45 fields a raw tuple would need an
unreadable \texttt{.2.2.2\ldots} chain to destructure, and \texttt{Nonempty} gives the identical
opacity guarantee more legibly. \texttt{encodePair}/\texttt{encodePair\_inj} (\ref{encodePair}) are the
one part of this cluster \emph{not} eliminated by this trick --- whether \texttt{Element} admits an
injective self-pairing is an independent, self-contained question about \texttt{Element}'s own
cardinality, unrelated to \texttt{Design}, and (2026-08-26, same session) was proved directly instead
(\ref{encodePair_inj}).

\texttt{\textcolor{leancolor}{class} \textcolor{leanyellow}{CoreDesignModel} \textcolor{leancolor}{where}}

\texttt{~~\ldots\ (all fields below, each shown at its own point in this chapter)}
\label{CoreDesignModel}
\end{restatable}

\begin{restatable}{leantheorem}{coreDesignModelNonemptyLean}~\leanTitle{coreDesignModel\_nonempty / coreDesignModelInstance}~
The consistency certificate for \texttt{CoreDesignModel} (\ref{CoreDesignModel}): a
\texttt{Nonempty CoreDesignModel} proof built from one concrete named-field term rather than an
$\exists$-tuple, per that entry's own reasoning above. \texttt{Design := $\emptyset$} and
\texttt{FeatureContent := fun \_ \_ => False} make every \texttt{df\_*} field's own hypothesis
unconstructible, so each is discharged individually (\texttt{by intro \ldots; exact absurd h (by simp)},
or \texttt{hb.elim} for \texttt{df\_featurechaining}'s \texttt{FeatureContent}-shaped hypothesis) rather
than assumed from the shared hypothesis shape alone; every non-constraint field left as an arbitrary
trivial value of the right type (\texttt{False} for \texttt{Prop}-valued fields, $\emptyset$ for
\texttt{Set Element}-valued ones). \texttt{df\_classifier2} (\texttt{VC $\subseteq$ VT}) is the one field
not resting on \texttt{Design}'s vacuity --- \texttt{VC := $\emptyset$} handles it directly.
\texttt{coreDesignModelInstance} then goes through \texttt{Classical.choice} over this proof, same
opacity guarantee as every other model in this project (\ref{model}, \ref{mereologyModel},
\ref{regionModelInstance}, \ref{surfaceModelInstance}) --- the actual field values chosen stay opaque;
this only certifies \emph{a} model exists. Full witness code omitted here as for
\ref{regionModelInstance}/\ref{surfaceModelInstance} --- at $\sim$45 fields it is mostly this same
one-line contradiction repeated, not illuminating to reproduce in full.
\label{coreDesignModel_nonempty}
\label{coreDesignModelInstance}
\end{restatable}
...
\begin{restatable}{leandefinition}{Specializes}~\leanTitle{Specializes}~ KerML's own textual \texttt{specializes} keyword,
...
\texttt{Design} is. Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~Specializes : Element $\to$ Element $\to$ Prop}
\label{Specializes}
\end{restatable}

\begin{restatable}{leandefinition}{dfSpecializes}~\leanTitle{df\_specializes}~ Define \texttt{specializes} in Lean.
Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously (\texttt{Design :=
$\emptyset$} makes \texttt{hs}/\texttt{hg} unconstructible).

\texttt{~~df\_specializes : $\forall$ \{ts tg : Element\} \{Cs Cg : Set Element\},}

\texttt{~~~~(DesignKind.type, ts, Cs) $\in$ Design $\to$ (DesignKind.type, tg, Cg) $\in$ Design $\to$}
...
\pn \texttt{df-multspec}'s own formula above already claims full $n$-ary generality ($t_s \subseteq
t_g \cap \ldots \cap t_j$ for arbitrarily many generalizations), but until 2026-09-09
\texttt{df\_multspec} only ever implemented the two-generalization case shown above.

\begin{restatable}{leantheorem}{dfMultspecNLean}~\leanTitle{df\_multspec\_n}~ \texttt{df\_multspec}'s
true $n$-ary generalization, indexed over \texttt{Fin n} rather than a fixed pair -- proved by the
identical one-line argument (\texttt{df\_specializes} applied to each generalization independently),
now closed under \texttt{Set.mem\_iInter} instead of manual pairing.

\texttt{\textcolor{leancolor}{theorem} \textcolor{leanyellow}{df\_multspec\_n} \{n : $\mathbb{N}$\} \{ts : Element\} \{Cs : Set Element\}}
...
\texttt{~~(hspec : $\forall$ i, Specializes ts (tg i)) :}

\texttt{~~Cs $\subseteq$ $\bigcap$ i, Cg i :=}

\texttt{~~by intro x hx; simp only [Set.mem\_iInter]; intro i; exact df\_specializes hs (hg i) (hspec i) hx}
\label{df_multspec_n}
\end{restatable}
...
\begin{restatable}{leandefinition}{Unions}~\leanTitle{Unions}~ KerML's own textual \texttt{unions} keyword, in Lean.
...
actually asserted in the current model. Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~Unions : Element $\to$ Element $\to$ Prop}
\label{Unions}
\end{restatable}

\begin{restatable}{leandefinition}{dfUnionsLean}~\leanTitle{df\_unions}~ Define \texttt{unions} in Lean.
Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously.

\texttt{~~df\_unions : $\forall$ \{ta tb tc : Element\} \{Ca Cb Cc : Set Element\},}

\texttt{~~~~(DesignKind.type, ta, Ca) $\in$ Design $\to$ (DesignKind.type, tb, Cb) $\in$ Design $\to$}

\texttt{~~~~(DesignKind.type, tc, Cc) $\in$ Design $\to$ Specializes ta tb $\to$ Unions ta tc $\to$ Ca = Cb $\cup$ Cc}
\label{df_unions}
\end{restatable}
...
\begin{restatable}{leandefinition}{Intersects}~\leanTitle{Intersects}~ KerML's own textual \texttt{intersects}
keyword, in Lean. Same reasoning as \texttt{Unions} above. Now a field of \texttt{CoreDesignModel}
(\ref{CoreDesignModel}).

\texttt{~~Intersects : Element $\to$ Element $\to$ Prop}
\label{Intersects}
\end{restatable}

\begin{restatable}{leandefinition}{dfIntersectsLean}~\leanTitle{df\_intersects}~ Define \texttt{intersects} in Lean.
Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously.

\texttt{~~df\_intersects : $\forall$ \{ta tb tc : Element\} \{Ca Cb Cc : Set Element\},}

\texttt{~~~~(DesignKind.type, ta, Ca) $\in$ Design $\to$ (DesignKind.type, tb, Cb) $\in$ Design $\to$}

\texttt{~~~~(DesignKind.type, tc, Cc) $\in$ Design $\to$ Specializes ta tb $\to$ Intersects ta tc $\to$ Ca = Cb $\cap$ Cc}
\label{df_intersects}
\end{restatable}
...
\begin{restatable}{leandefinition}{Differences}~\leanTitle{Differences}~ KerML's own textual \texttt{differences}
keyword, in Lean. Same reasoning as \texttt{Unions} above. Now a field of \texttt{CoreDesignModel}
(\ref{CoreDesignModel}).

\texttt{~~Differences : Element $\to$ Element $\to$ Prop}
\label{Differences}
\end{restatable}

\begin{restatable}{leandefinition}{dfDifferencesLean}~\leanTitle{df\_differences}~ Define \texttt{differences} in
Lean. Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously.

\texttt{~~df\_differences : $\forall$ \{ta tb tc : Element\} \{Ca Cb Cc : Set Element\},}

\texttt{~~~~(DesignKind.type, ta, Ca) $\in$ Design $\to$ (DesignKind.type, tb, Cb) $\in$ Design $\to$}

\texttt{~~~~(DesignKind.type, tc, Cc) $\in$ Design $\to$ Specializes ta tb $\to$ Differences ta tc $\to$ Ca = Cb $\setminus$ Cc}
...
\begin{restatable}{leandefinition}{Disjoint}~\leanTitle{Disjoint}~ KerML's own textual \texttt{disjoint from}
keyword, in Lean. Same reasoning as \texttt{Unions} above. Now a field of \texttt{CoreDesignModel}
(\ref{CoreDesignModel}).

\texttt{~~Disjoint : Element $\to$ Element $\to$ Prop}
\label{Disjoint}
\end{restatable}

\begin{restatable}{leandefinition}{dfDisjointLean}~\leanTitle{df\_disjoint}~ Define \texttt{disjoint} in Lean. The
...
\texttt{~~df\_disjoint : $\forall$ \{ta tb tc : Element\} \{Ca Cb Cc : Set Element\},}

\texttt{~~~~(DesignKind.type, ta, Ca) $\in$ Design $\to$ (DesignKind.type, tb, Cb) $\in$ Design $\to$}

\texttt{~~~~(DesignKind.type, tc, Cc) $\in$ Design $\to$ Specializes ta tb $\to$ Disjoint ta tc $\to$}

\texttt{~~~~Ca $\subseteq$ Cb $\wedge$ Ca $\cap$ Cc = $\emptyset$}
\label{df_disjoint}
\end{restatable}
...
\pn Checked against \texttt{Core.lean}'s own concrete-syntax grammar for \texttt{type}: none of the three
listings above originally parsed there. \texttt{kermlType} (\texttt{Core.lean}, KerML \S8.2.4.1.1's own
relationship-part subset) hardcoded clause order --- \texttt{specializes}, \texttt{conjugates},
\texttt{disjoint from}, \texttt{unions}, \texttt{intersects}, \texttt{differences}, always in that sequence
--- rather than accepting the relationship parts in any order the way real KerML concrete syntax (and this
section's own listings) does. Fixed the same day: see \texttt{kermlTypeRelPart} below.

\begin{restatable}{leandefinition}{kermlTypeRelPartLean}~\leanTitle{kermlTypeRelPart}~ \texttt{Core.lean}'s
\texttt{Type} relationship parts (\texttt{disjoint from}/\texttt{unions}/\texttt{intersects}/
\texttt{differences}), promoted from four fixed-position optional slots to their own freely-orderable,
repeatable syntax category --- matching real KerML concrete syntax's own ``any number of
\texttt{TypeRelationshipPart}, in any order'' (this section's own opening citation), and letting all three
listings above actually parse now. The elaborator also builds its flat \texttt{Element} list in the order
the parts appear in the source, not bucketed by kind first, as it did before --- confirmed via \texttt{\#check}
(both \texttt{Core} alone and the whole project, a real \texttt{lake build}) that the three listings' own
output element order now matches each listing's own left-to-right clause order. \texttt{kermlClassifier}
reuses the same category, same fix.
\label{kermlTypeRelPart}
\end{restatable}

\pn The theorem below is independent of that grammar fix: it formalizes this section's own argument at the
level of what these declarations actually \emph{mean} (\texttt{Set Element} composition), not just that the
grammar now happens to accept all three spellings.

\begin{restatable}{leantheorem}{typeOperatorCompositionOrderMattersLean}~\leanTitle{typeOperatorCompositionOrderMatters}~
The formal version of this section's own ``Certainly not'': a concrete witness where $R_1$/$R_2$/$R_3$ (the
three listings' formulas above, restated as \texttt{Set Element} expressions) are pairwise distinct, proving
operator order is genuinely load-bearing, not just cosmetically different concrete syntax for the same
abstract result. Witnessed by $B=\emptyset$, $C=\{p,q\}$, $D=E=\{q\}$, $F=\emptyset$ for two fresh
\texttt{Element}s $p\ne q$ used only as set-membership probes: $R_1=\emptyset$, $R_2=\{p,q\}$, $R_3=\{q\}$,
pairwise distinct as required. Verified via a real \texttt{lake build} (both \texttt{Core} alone and the
whole project), not just checked by eye.

\texttt{\textcolor{leancolor}{theorem} \textcolor{leanyellow}{typeOperatorCompositionOrderMatters} :}

\texttt{~~$\exists$ B C D E F : Set Element,}

\texttt{~~~~~~(((B $\cup$ C) $\cap$ D) $\setminus$ E) $\setminus$ F $\ne$ (((B $\cap$ D) $\setminus$ E) $\cup$ C) $\setminus$ F $\wedge$}

\texttt{~~~~~~(((B $\cup$ C) $\cap$ D) $\setminus$ E) $\setminus$ F $\ne$ (((B $\setminus$ E) $\setminus$ F) $\cup$ C) $\cap$ D $\wedge$}

\texttt{~~~~~~(((B $\cap$ D) $\setminus$ E) $\cup$ C) $\setminus$ F $\ne$ (((B $\setminus$ E) $\setminus$ F) $\cup$ C) $\cap$ D}
\label{typeOperatorCompositionOrderMatters}
\end{restatable}
...
\begin{restatable}{leandefinition}{TypeMultiplicityExact}~\leanTitle{TypeMultiplicityExact}~ KerML's exact
multiplicity bracket \texttt{[n]}, in Lean. A fresh \texttt{Element}-level primitive, same reasoning as
\texttt{Specializes}: nothing currently represents a type's declared multiplicity. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~TypeMultiplicityExact : Element $\to$ $\mathbb{N}$ $\to$ Prop}
\label{TypeMultiplicityExact}
\end{restatable}

\begin{restatable}{leandefinition}{dfTypemultiplicityLean}~\leanTitle{df\_typemultiplicity}~ Define type
...
not because it's independently new content. Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}),
proven vacuously.

\texttt{~~df\_typemultiplicity : $\forall$ \{A B : Element\} \{Ca Cb : Set Element\} \{n : $\mathbb{N}$\},}

\texttt{~~~~(DesignKind.type, A, Ca) $\in$ Design $\to$ (DesignKind.type, B, Cb) $\in$ Design $\to$}
...
\begin{restatable}{leandefinition}{TypeMultiplicityRange}~\leanTitle{TypeMultiplicityRange}~ KerML's multiplicity
range bracket \texttt{[m..n]}, in Lean. Same reasoning as \texttt{TypeMultiplicityExact} above. Now a field
of \texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~TypeMultiplicityRange : Element $\to$ $\mathbb{N}$ $\to$ $\mathbb{N}$ $\to$ Prop}
\label{TypeMultiplicityRange}
\end{restatable}

\begin{restatable}{leandefinition}{dfTypemultiplicityrangeLean}~\leanTitle{df\_typemultiplicityrange}~ Define type
multiplicity range in Lean. Same reasoning as \texttt{df\_typemultiplicity} above. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously.

\texttt{~~df\_typemultiplicityrange : $\forall$ \{A B : Element\} \{Ca Cb : Set Element\} \{m n : $\mathbb{N}$\},}
...
\begin{restatable}{leandefinition}{VF}~\leanTitle{VF}~ Define the set of \texttt{Features} in Lean. Exact parallel of
\texttt{VT} above, one level down. Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~VF : Set Element}
\label{VF}
\end{restatable}

\begin{restatable}{leandefinition}{dfFeatureLean}~\leanTitle{df\_feature}~ Define \texttt{feature} in Lean. Unlike
\texttt{type}, \texttt{Feature} already has a slot in \texttt{ElementKind}'s fourteen tags, so no new
\texttt{DesignKind} case is needed --- exact parallel of \texttt{df\_types} above. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously.

\texttt{~~df\_feature : $\forall$ e : Element,}
...
\begin{restatable}{leandefinition}{FeatureContent}~\leanTitle{FeatureContent}~ A feature's design-triple content, in
...
into the \texttt{(DesignKind, Element, Set Element)} triple shape used everywhere else. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~FeatureContent : Element $\to$ Set (Element $\times$ Element) $\to$ Prop}
\label{FeatureContent}
\end{restatable}

\begin{restatable}{leandefinition}{FeaturedByDecl}~\leanTitle{FeaturedByDecl}~ \texttt{feature x typed by Y featured
by Z;} in Lean. A fresh \texttt{Element}-level primitive, same reasoning as \texttt{Specializes}: nothing
currently represents this concrete-syntax clause. Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~FeaturedByDecl : Element $\to$ Element $\to$ Element $\to$ Prop}
\label{FeaturedByDecl}
\end{restatable}

\begin{restatable}{leandefinition}{dfFeaturedbyLean}~\leanTitle{df\_featuredby}~ Define \texttt{featured by} in Lean.
Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously.

\texttt{~~df\_featuredby : $\forall$ \{x y z : Element\} \{Cy Cz : Set Element\},}

\texttt{~~~~(DesignKind.type, y, Cy) $\in$ Design $\to$ (DesignKind.type, z, Cz) $\in$ Design $\to$}
...
\begin{restatable}{leandefinition}{DomainlessDecl}~\leanTitle{DomainlessDecl}~ \texttt{feature x typed by Y;} (no
...
plus a ``no featured-by clause'' side condition. Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~DomainlessDecl : Element $\to$ Element $\to$ Prop}
\label{DomainlessDecl}
\end{restatable}

\begin{restatable}{leandefinition}{dfDomainlessLean}~\leanTitle{df\_domainless}~ Define domain-less \texttt{Feature}
...
\texttt{RegionSurface}/\texttt{RegionFilm} in \S0.8. Now a field of \texttt{CoreDesignModel}
(\ref{CoreDesignModel}), proven vacuously.

\texttt{~~df\_domainless : $\forall$ \{x y : Element\} \{Cy : Set Element\}, (DesignKind.type, y, Cy) $\in$ Design $\to$}
...
\begin{restatable}{leandefinition}{FeatureMemberOf}~\leanTitle{FeatureMemberOf}~ A type or feature $Z$'s owned
...
Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~FeatureMemberOf : Element $\to$ Element $\to$ Set (Element $\times$ Element) $\to$ Prop}
\label{FeatureMemberOf}
\end{restatable}

\begin{restatable}{leandefinition}{OwnedFeatureTypedBy}~\leanTitle{OwnedFeatureTypedBy}~ \texttt{type Z \{ feature x
typed by Y; \}} (an owned feature member, no explicit \texttt{featured by}), in Lean. A fresh
\texttt{Element}-level primitive, same reasoning as \texttt{Specializes}. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~OwnedFeatureTypedBy : Element $\to$ Element $\to$ Element $\to$ Prop}
\label{OwnedFeatureTypedBy}
\end{restatable}

\begin{restatable}{leandefinition}{dfFeaturememberLean}~\leanTitle{df\_featuremember}~ Define \texttt{Feature
...
extension (co-domain). Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously.

\texttt{~~df\_featuremember : $\forall$ \{Z T x Y' : Element\} \{Ct Cy : Set Element\},}

\texttt{~~~~(DesignKind.type, T, Ct) $\in$ Design $\to$ (DesignKind.type, Y', Cy) $\in$ Design $\to$}
...
\begin{restatable}{leandefinition}{encodePair}~\leanTitle{encodePair}~ Kuratowski-style pairing, in Lean, encoding
...
need its own primitive. \textbf{Proved, not axiomatized (2026-08-26)} --- unlike every other primitive in
this chapter, this one isn't part of \texttt{CoreDesignModel}'s \texttt{Design := $\emptyset$} vacuity trick
(it doesn't mention \texttt{Design} at all): \texttt{Element}'s own fields (\texttt{String}/\texttt{List
String}/\texttt{Option String} $\times 2$) each admit an injection into $\mathbb{N}$ (\texttt{String} via
\texttt{Char}'s \texttt{UInt32} codepoint folded through a hand-written Gödel-style \texttt{List $\mathbb{N}$
$\to$ $\mathbb{N}$} encoding; \texttt{List}/\texttt{Option} generically over an already-injective element
type), so \texttt{Element} itself injects into $\mathbb{N}$, and \texttt{Nat.pair} (Mathlib's own
$\mathbb{N} \times \mathbb{N} \simeq \mathbb{N}$) makes the pairing of two such naturals injective. The
result folds back into a real \texttt{Element} via \texttt{aliasIds := List.replicate n ""}, injective in
\texttt{n} via \texttt{List.length} alone.

\texttt{\textcolor{leancolor}{def} \textcolor{leanyellow}{encodePair} (a b : Element) : Element :=}

\texttt{~~$\{$elementId := "", aliasIds := List.replicate (Nat.pair (elementToNat a) (elementToNat b)) "",}

\texttt{~~~~declaredName := none, declaredShortName := none$\}$}
\label{encodePair}
\end{restatable}

\begin{restatable}{leantheorem}{encodePairInj}~\leanTitle{encodePair\_inj}~ \texttt{encodePair} is injective:
distinct pairs give distinct encodings -- now a genuine theorem (\ref{encodePair}'s own doc comment), proved
via \texttt{List.length\_replicate} recovering \texttt{Nat.pair (elementToNat a) (elementToNat b)} from
\texttt{encodePair a b}'s \texttt{aliasIds} field, then \texttt{Nat.pair\_eq\_pair} and the injectivity of
\texttt{elementToNat}. Proof omitted here.

\texttt{\textcolor{leancolor}{theorem} \textcolor{leanyellow}{encodePair\_inj} \{a b c d : Element\} (h : encodePair a b = encodePair c d) :}

\texttt{~~a = c $\wedge$ b = d}
\label{encodePair_inj}
\end{restatable}

\begin{restatable}{leandefinition}{dfFeatureoffeatureLean}~\leanTitle{df\_featureoffeature}~ Define \texttt{featured
...
relation pairs $x$'s own encoded pairs (domain $v$, codomain from $Y$) with $Z$'s extension. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously.

\texttt{~~df\_featureoffeature : $\forall$ \{x w Y' Z' : Element\} \{Cy Cz : Set Element\},}
...
\begin{gather}
\mc{Person} = \{~\langle \textbf{type},w,\mc{C}\rangle~\vert~w=W(``Person")~\wedge \{y_1,y_2 \} \subseteq \mc{Person} \notag \\
...
\mc{Z}=\{~\langle \textbf{type},w,\mc{C}\rangle~\vert~w=W(``Z")~\wedge \{y_1,\ldots,y_k \} \subseteq \mc{Y} \notag \\
...
\begin{restatable}{leandefinition}{OwnedFeatureTypedByExact}~\leanTitle{OwnedFeatureTypedByExact}~ \texttt{type Z \{
feature x[k] typed by Y; \}} (exact feature multiplicity), in Lean. Now a field of \texttt{CoreDesignModel}
(\ref{CoreDesignModel}).

\texttt{~~OwnedFeatureTypedByExact : Element $\to$ Element $\to$ Element $\to$ $\mathbb{N}$ $\to$ Prop}
\label{OwnedFeatureTypedByExact}
\end{restatable}

\begin{restatable}{leandefinition}{dfFixedfeaturemultiplicityLean}~\leanTitle{df\_fixedfeaturemultiplicity}~ Define
...
way \texttt{cardElem}/\texttt{df\_typemultiplicity} did in \S2.2.3, just for pair-sets. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously.

\texttt{~~df\_fixedfeaturemultiplicity : $\forall$ \{Z x Y' : Element\} \{Cy : Set Element\}}

\texttt{~~~~\{R : Set (Element $\times$ Element)\} \{k : $\mathbb{N}$\}, (DesignKind.type, Y', Cy) $\in$ Design $\to$}
...
\begin{restatable}{leandefinition}{OwnedFeatureTypedByStar}~\leanTitle{OwnedFeatureTypedByStar}~ \texttt{type Z \{
feature x[*] typed by Y; \}} (unbounded feature multiplicity), in Lean. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~OwnedFeatureTypedByStar : Element $\to$ Element $\to$ Element $\to$ Prop}
\label{OwnedFeatureTypedByStar}
\end{restatable}

\begin{restatable}{leandefinition}{dfUndefinedfeaturemultiplicityLean}~\leanTitle{df\_undefinedfeaturemultiplicity}~
Define \texttt{Undefined Feature Multiplicity} in Lean: an unbounded feature member's relation has every
pair's co-domain drawn from $Y$'s extension, with no cardinality constraint at all. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously.

\texttt{~~df\_undefinedfeaturemultiplicity : $\forall$ \{Z x Y' : Element\} \{Cy : Set Element\}}

\texttt{~~~~\{R : Set (Element $\times$ Element)\}, (DesignKind.type, Y', Cy) $\in$ Design $\to$}
...
\begin{restatable}{leandefinition}{OwnedFeatureTypedByRange}~\leanTitle{OwnedFeatureTypedByRange}~ \texttt{type Z \{
feature x[l..u] typed by Y; \}} (feature multiplicity range), in Lean. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~OwnedFeatureTypedByRange : Element $\to$ Element $\to$ Element $\to$ $\mathbb{N}$ $\to$ $\mathbb{N}$ $\to$ Prop}
\label{OwnedFeatureTypedByRange}
\end{restatable}

\begin{restatable}{leandefinition}{dfFeaturemultiplicityrangeLean}~\leanTitle{df\_featuremultiplicityrange}~ Define
\texttt{Feature Multiplicity Range} in Lean: a range-bounded feature member's relation has cardinality
between $l$ and $u$, every pair's co-domain drawn from $Y$'s extension. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously.

\texttt{~~df\_featuremultiplicityrange : $\forall$ \{Z x Y' : Element\} \{Cy : Set Element\}}

\texttt{~~~~\{R : Set (Element $\times$ Element)\} \{l u : $\mathbb{N}$\}, (DesignKind.type, Y', Cy) $\in$ Design $\to$}
...
\begin{restatable}{leandefinition}{OwnedFeatureTypedByRangeStar}~\leanTitle{OwnedFeatureTypedByRangeStar}~
\texttt{type Z \{ feature x[l..*] typed by Y; \}} (feature multiplicity range, unbounded upper end), in Lean.
Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~OwnedFeatureTypedByRangeStar : Element $\to$ Element $\to$ Element $\to$ $\mathbb{N}$ $\to$ Prop}
\label{OwnedFeatureTypedByRangeStar}
\end{restatable}

\begin{restatable}{leandefinition}{dfFeaturemultiplicityrangestarLean}~\leanTitle{df\_featuremultiplicityrangestar}~
Define \texttt{Feature Multiplicity Range Star} in Lean: same as \texttt{df\_featuremultiplicityrange} above
but with only a lower bound $l$. Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven
vacuously.

\texttt{~~df\_featuremultiplicityrangestar : $\forall$ \{Z x Y' : Element\} \{Cy : Set Element\}}

\texttt{~~~~\{R : Set (Element $\times$ Element)\} \{l : $\mathbb{N}$\}, (DesignKind.type, Y', Cy) $\in$ Design $\to$}
...
\begin{restatable}{leandefinition}{MemberOf}~\leanTitle{MemberOf}~ \texttt{type B \{ member feature g typed by C
...
\emph{not} an owned feature of $B$. Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}) -- no
\texttt{df\_*} axiom of its own even before this conversion (\texttt{df\_membershipfeature} below is
already a real theorem not needing one).

\texttt{~~MemberOf : Element $\to$ Element $\to$ Prop}
\label{MemberOf}
\end{restatable}
...
\begin{restatable}{leandefinition}{OwnedFeatures}~\leanTitle{OwnedFeatures}~ \texttt{type Z \{ feature x1 typed by
...
a literal ``...'' ellipsis for a fixed but unspecified arity $j$. Now a field of \texttt{CoreDesignModel}
(\ref{CoreDesignModel}).

\texttt{~~OwnedFeatures : Element $\to$ List (Element $\times$ Element) $\to$ Prop}
\label{OwnedFeatures}
\end{restatable}

\begin{restatable}{leandefinition}{dfTypebodyLean}~\leanTitle{df\_typebody}~ Define multiple \texttt{TypeBody}
...
member-by-member'', which is exactly what quantifying over \texttt{feats} states. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}), proven vacuously (\texttt{hz} unconstructible).

\texttt{~~df\_typebody : $\forall$ \{Z : Element\} \{feats : List (Element $\times$ Element)\} \{Cz : Set Element\},}

\texttt{~~~~(DesignKind.type, Z, Cz) $\in$ Design $\to$ OwnedFeatures Z feats $\to$}
...
\begin{restatable}{leandefinition}{OrderedOn}~\leanTitle{OrderedOn}~ \texttt{OrderedOn Y a b} means $a<b$ under
...
global order on \texttt{Element}. Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}) -- no
\texttt{df\_*} axiom of its own (only used as a hypothesis inside the already-theorem \texttt{df\_ordered}
below).

\texttt{~~OrderedOn : Element $\to$ Element $\to$ Element $\to$ Prop}
\label{OrderedOn}
\end{restatable}

\begin{restatable}{leandefinition}{OrderedDecl}~\leanTitle{OrderedDecl}~ \texttt{type Z \{ feature x[*] ordered
typed by Y; \}}, in Lean. Now a field of \texttt{CoreDesignModel} (\ref{CoreDesignModel}), same reasoning as
\texttt{OrderedOn}.

\texttt{~~OrderedDecl : Element $\to$ Element $\to$ Element $\to$ Prop}
\label{OrderedDecl}
\end{restatable}
...
\begin{restatable}{definition}{dffeatureinverting}~\blTitle{df-featureinverting}~   Define \texttt{Feature Inverting}
\begin{align}
...
\begin{restatable}{leandefinition}{ChainsDecl}~\leanTitle{ChainsDecl}~ \texttt{feature a chains b.c;}, in Lean. A
fresh \texttt{Element}-level primitive, same reasoning as \texttt{Specializes}. Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}).

\texttt{~~ChainsDecl : Element $\to$ Element $\to$ Element $\to$ Prop}
\label{ChainsDecl}
\end{restatable}

\begin{restatable}{leandefinition}{dfFeaturechainingLean}~\leanTitle{df\_featurechaining}~ Define \texttt{Feature
Chaining} in Lean: a feature $a$ declared as chaining $b.c$ has content the composition of $c$'s and $b$'s own
relations (via \texttt{relCompose}, defined just below at \texttt{df-co}). Now a field of
\texttt{CoreDesignModel} (\ref{CoreDesignModel}) -- its hypotheses are \texttt{FeatureContent}-shaped, not
\texttt{Design}-membership-shaped, unlike every other constraint above, so it's discharged via
\texttt{FeatureContent}'s own vacuity (\texttt{FeatureContent := fun \_ \_ => False}) rather than
\texttt{Design}'s.

\texttt{~~df\_featurechaining : $\forall$ \{a b c : Element\} \{Rb Rc : Set (Element $\times$ Element)\},}

\texttt{~~~~FeatureContent b Rb $\to$ FeatureContent c Rc $\to$}
